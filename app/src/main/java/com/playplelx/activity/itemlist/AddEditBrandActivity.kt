...
class AddEditBrandActivity : AppCompatActivity(), View.OnClickListener{
        ...
        if (intent.extras != null) {
            ...
            if (mFrom == Constants.isEdit) {
                ...
                brandModel = Gson().fromJson(
                    intent.getStringExtra(Constants.brandModel),
                    BrandModel::class.java
                )
                setData(brandModel)
            } else {
    ...
    private fun isValidate(): Boolean {
        ...
        if (fileUrl.isEmpty()) {
            ...
            isValid = false
        } else if (edtBrandName.text.toString().trim().isEmpty()) {
    ...
    private fun addBrand() {
        ...
        val call = apiInterface.addBrand(
            ...
            edtBrandName.text.toString().trim().lowercase(), file, fileUrl
        )
        call.enqueue(object : Callback<JsonObject> {
            override fun onResponse(call: Call<JsonObject>, response: Response<JsonObject>) {
                if (response.isSuccessful) {
