...
import com.playplexmatm.activity.AddProductsActivity
import com.playplexmatm.activity.PurchasePaymentModeActivity
...
import kotlinx.android.synthetic.main.activity_add_sale.acOrderStatus
import kotlinx.android.synthetic.main.activity_add_sale.edtBasicAmount
import kotlinx.android.synthetic.main.activity_add_sale.ivAddProduct
import kotlinx.android.synthetic.main.activity_add_sale.ivAddSupplier
import kotlinx.android.synthetic.main.activity_add_sale.llAddItemForm
import kotlinx.android.synthetic.main.activity_add_sale.rl_add_item
import kotlinx.android.synthetic.main.activity_add_sale.rl_amount
import kotlinx.android.synthetic.main.activity_add_sale.tvBasicAmount
import kotlinx.android.synthetic.main.activity_add_sale.tvBasicAmountTAG1
import kotlinx.android.synthetic.main.activity_add_sale.tvDirect
import kotlinx.android.synthetic.main.activity_add_sale.tvDiscountTotal
import kotlinx.android.synthetic.main.activity_add_sale.tvItemDiscount
import kotlinx.android.synthetic.main.activity_add_sale.tvOrderTax
import kotlinx.android.synthetic.main.activity_add_sale.tvProduct
import kotlinx.android.synthetic.main.activity_add_sale.tvQuantity
import kotlinx.android.synthetic.main.activity_add_sale.tvShipppingTotal
...
class AddSaleActivity : AppCompatActivity(), View.OnClickListener,
    FilterProductAdapter.onClick, SupplierListAdapter.ListAdapterListener,
    UsersAdapter.ListAdapterListener, WarehouseListAdapter.ListAdapterListener,
    AppApiCalls.OnAPICallCompleteListener {

    lateinit var mContext: AddSaleActivity
    private var supplierStringList: ArrayList<String> = arrayListOf()
    private var orderStatusList: ArrayList<String> = arrayListOf("received","pending","ordered")
    lateinit var purchaseModel: SaleListModel
    ...
    lateinit var wareHouseAdapter: WarehouseListAdapter
...
//    lateinit var acOrderTax: AutoCompleteTextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(bundle != null) {
            purchaseModel = bundle.getSerializable("model") as SaleListModel
            from = "edit"
            ...
            acUserName.setText(purchaseModel.user.name)
            acWareHouse.setText(purchaseModel.staff_member.name)

            wareHouseId = purchaseModel.staff_member.xid
...
//                            setAdapter(userList)
    ...
    private fun showUsersBottomSheet()
    {
        ...
        view.rvCustomer.apply {
            ...
            usersAdapter = UsersAdapter(
                context, userList, this@AddSaleActivity
            )
            view.rvCustomer.adapter = usersAdapter
        }
    ...
    private fun showWarehouseBottomSheet()
    {
        ...
        view.rvCustomer.apply {
            ...
            layoutManager = LinearLayoutManager(this@AddSaleActivity)
            wareHouseAdapter = WarehouseListAdapter(
                context, wareHouseList, this@AddSaleActivity
            )
            view.rvCustomer.adapter = wareHouseAdapter
        }
        view.etSearch.addTextChangedListener(object : TextWatcher {
            override fun beforeTextChanged(
                s: CharSequence?,
    ...
    fun filterWarehouse(text: String) {
        val temp: MutableList<WareHouseModel> = ArrayList()
        for (d in wareHouseList) {
            //or use .equal(text) with you want equal match
            //use .toLowerCase() for better matches
            if (d.name.contains(text, ignoreCase = true) || d.phone.contains(text, ignoreCase = true)) {
                temp.add(d)
        ...
        //update recyclerview
        wareHouseAdapter.updateList(temp)
    }
...
//
    ...
    private fun mNetworkCallGetSuppliersAPI() {
        ...
        call.enqueue(object : Callback<JsonObject> {
            ...
            override fun onFailure(call: Call<JsonObject>, t: Throwable) {
                pbLoadData.visibility = View.GONE
                Toast.makeText(this@AddSaleActivity, t.message, Toast.LENGTH_SHORT).show()
    ...
    private fun setwareHouseAdapter(wareHouseList: ArrayList<WareHouseModel>) {
        ...
        acWareHouse.setText(wareHouseList[0].name)
        wareHouseId = wareHouseList[0].xid
...
//                                }
                                ...
                                if(type.equals("direct")) {
                                    mNetworkCallAddSaleAPI(0)
                                } else {
                                    mNetworkCallAddSaleAPI(1)
                                }
                            }
                ...
                if (InternetConnection.checkConnection(mContext)) {
                    if (isValidate()) {
                        if(from.equals("edit")) {
                            ...
                            if(type.equals("direct")) {
                                mNetworkCallAddSaleViewPdfAPI(0)
                            } else {
                                mNetworkCallAddSaleViewPdfAPI(1)
                            }
                        }
                    }
                } else {
...
//            }
    ...
    private fun mNetworkCallAddSaleAPI(is_product_invoice: Int) {
        pbLoadData.visibility = View.VISIBLE
...
//                            mNetworkCallGetSaleDetailsAPI(jsonObject.getJSONObject("data").getString("xid"))
                            val intent = Intent()
    ...
    private fun mNetworkCallAddSaleViewPdfAPI(is_product_invoice: Int) {
        pbLoadData.visibility = View.VISIBLE
    ...
    private fun mNetworkCallUpdateSaleAPI() {
        pbLoadData.visibility = View.VISIBLE
        ...
        val call = apiInterface.updateSales(
            "sales",
            edtInvoiceNumber.text.toString().trim(),
            edtDate.text.toString().trim(),
            wareHouseId,
...
//        Log.e("Temp Tax Rate",tempTaxRate.toString())
    ...
    override fun onDeleteClick(filterModel: ProductFilterModel, position: Int) {
        ...
        if (filterProductList.size > 0) {
            ...
            tvBasicAmount.setText(resources.getString(R.string.Rupee)+" "+(DecimalFormat("##.##").format(basicAmount)).toString())
            tvItemDiscount.setText(resources.getString(R.string.Rupee)+" "+individualDiscountTotal)
    ...
    private fun getSupplierId() : String
    {
        for(i in 0 until supplierArrayList.size)
        {
            if( acUserName.text.toString().equals( supplierArrayList.get(i).name))
            {
    ...
    private fun mNetworkCallGetSaleDetailsAPI(id: String) {
        pbLoadData.visibility = View.VISIBLE
        val call = apiInterface.getSalesId(id)
        call.enqueue(object : Callback<JsonObject> {
            override fun onResponse(call: Call<JsonObject>, response: Response<JsonObject>) {
                if (response.isSuccessful) {
                    pbLoadData.visibility = View.GONE
                    if (response.code() == 200) {
                        ...
                        if (jsonObject.optBoolean("status")) {
                            ...
                            startActivity(i)
                            finish()
//                            startActivity(Intent(mContext,WebviewActivity::class.java).putExtra("data",data))
//                            finish()
