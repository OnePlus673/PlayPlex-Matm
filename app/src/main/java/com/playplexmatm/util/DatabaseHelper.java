...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public static final String DATABASE_NAME = "Payplexpos.db";
    ...
    public double getTotalCartAmt(PrefManager session) {
        ...
        session.setValue(Constants.KEY_Ordervid, vidlist.toString());
        session.setValue(Constants.KEY_Orderqty, qtylist.toString());
        session.setValue(Constants.KEY_Ordername, namelist.toString());
    ...
    @SuppressLint("Range")
    public Double CheckItemsTotal(String vid, String pid, double price, double quantity) {
        ...
        if (cursor.moveToFirst()) {
            ...
            count = decimalformatData.format(total);
            count = cursor.getString(cursor.getColumnIndex(TOTAL_PRICE));
            if (count.equals("0")) {
                db.execSQL("DELETE FROM " + TABLE_ORDER_NAME + " WHERE " + VID + " = ? AND " + PID + " = ?", new String[]{vid, pid});
    ...
    public String getSum() {
        ...
        String sAmount = "0";
        String sQuery = "select sum(totalprice) from " + TABLE_ORDER_NAME;
        Cursor cursor = database.rawQuery(sQuery, null);
        if (cursor.moveToFirst()) {
            sAmount = cursor.getString(cursor.getColumnIndex(TOTAL_PRICE));
        }
        return sAmount;
    }
